...
def codeInp(In):    #converts expression in string format to infix format
    i = 0       #index of input In
    Out = []    #output infix expression
    while (i < len(In)):
...
def precedence (optr):  #returns precedence order of operators
    if (optr == '~'):   #unary minus / negation
        return 7
    elif (optr.isalpha()):
        return 6
    elif (optr == '^'):
...
def Infix_2_Postfix (In): #converts infix expression to its postfix form
    stack = []
    Out = []    #output array
    ...
    def pushOptr ():
        if (In[i] == '-'):
            ...
            elif (type(In[i-1]) != float) and (In[i-1] not in ')xXyY'):
                stack.append('~')
            else:
            ...
            if (i != 0):
                stack.append('+')
            elif (type(In[i-1])==float) or (In[i-1] in 'xXyY'):
                stack.append('+')
        else:
            stack.append(In[i])

    for i in range(len(In)):
        if (type(In[i]) == float) or (In[i] in 'xXyY'):
            Out.append(In[i])
        elif (In[i]== '('):
            stack.append(In[i])
        elif (In[i]== ')'):
